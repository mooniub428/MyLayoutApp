...
class MainDashboardActivityCopy : AppCompatActivity() {
    val lecturesArray = arrayOf(
        "Linear Layout",
        "Relative Layout",
        "Intro UI(Button,EditText-input,TextView)",
        "Intent(Explicit,Implicit)",
        "DialogBox",
        "ActivityLifeCycle",
        "List View(ArrayAdapter)",
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_main_dashboard_copy)

        val lectureListView: ListView = findViewById(R.id.lect_list)
        val arrayAdapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, lecturesArray)

        lectureListView.adapter = arrayAdapter
        lectureListView.onItemClickListener = AdapterView.OnItemClickListener { parent, view, position, id ->
            val selectedItem = parent.getItemAtPosition(position) as String
            Toast.makeText(this,"Selected Item ==> $selectedItem",Toast.LENGTH_LONG).show()

            // Create intents for each activity
            val intent = when (selectedItem) {
                "Linear Layout" -> Intent(this, LinearLayoutActivity::class.java)
                "Relative Layout" -> Intent(this, RelativeLayoutActivity::class.java)
                "Intro UI(Button,EditText-input,TextView)" -> Intent(this, IntroUIActivity::class.java)
                "Intent(Explicit,Implicit)" -> Intent(this, IntentActivity::class.java)
                "DialogBox" -> Intent(this, DialogBoxActivity::class.java)
                "ActivityLifeCycle" -> Intent(this, ActivityLifeCycle::class.java)
                "List View(ArrayAdapter)" -> Intent(this, ListViewActivity::class.java)
                "Spinner (ArrayAdapter)" -> Intent(this, SpinnerActivity::class.java)
                "Custom List View" -> Intent(this, CustomListViewActivity::class.java)
                else -> {
                    // Handle unknown selection
                    Toast.makeText(this,"Activity for $selectedItem not found",Toast.LENGTH_LONG).show()
                    return@OnItemClickListener // Exit the listener early
                }
            }
